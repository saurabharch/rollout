...
router.get('/customer/account', async (req, res) => {
    ...
    if(!req.session.customerPresent){
        res.redirect('/index/customer/login');
        return;
    }
    ...
    .sort({ orderDate: -1 })
    .toArray();
    res.render(`${config.themeViews}/index/customer-account`, {
        title: 'Orders',
        session: req.session,
        orders,
...
router.get('/admin/customer/view/:id?', restrict, async (req, res) => {
    ...
    if(!customer){
        ...
        return res.redirect('/index/admin/customers');
    }
    ...
    return res.render('/index/customer', {
        title: 'View customer',
        result: customer,
        admin: true,
        session: req.session,
...
router.get('/customer/login', async (req, res, next) => {
    ...
    res.render(`${config.themeViews}/index/customer-login`, {
...
router.post('/customer/reset/:token', async (req, res) => {
    ...
    if(!customer){
        req.session.message = 'Password reset token is invalid or has expired';
        req.session.message_type = 'danger';
        return res.redirect('/index/forgot');
    }
    ...
    try{
        ...
        return res.redirect('/index/checkout/payment');
    }catch(ex){
