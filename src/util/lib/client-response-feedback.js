...
const RequestResponseTime = async(request, response, next) =>{
    try {

        const url = new url(`${request.protocol}://${request.get('host')}${request.originalUrl}`);
        // Getting timings
        console.log(`Response Test Report:  ${url}`)
      return request(Object.assign(url.parse(url), {
        headers: {
            'User-Agent': 'Rollout-Server'
        }
        }), (err, res) => {
         (err || res.timings)
        });
...
module.exports = RequestResponseTime;