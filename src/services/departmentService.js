  ...
  return new Promise(function (resolve, reject) {
    ...
    // db.getCollection('requests').find({id_project: "5c12662488379d0015753c49", participants: { $exists: true, $ne: [] }}).sort({_id:-1}).limit(1)
      ...
      Request.find(query).sort({_id:-1}).limit(1).exec(function (err, lastRequests) {
          ...
          //   at /Users/andrealeo/dev/chat21/rollout-server/services/requestService.js:55:56
  ...
  return new Promise(function (resolve, reject) {
    ...
    return Project.findOne({_id: projectid, status: 100}, function(err, project) {
      ...
      return Department.findOne(query).exec(function (err, department) {
        ...
        // NOTE: TO TEST '?nobot = true' see in the rollout dashboard: mongodb-department.service > testAssignesFunction
        if (nobot) {
