try {
  var _pgSd = {
    site: {
      sI: "https://assetscdn.pushengage.com/site_images/f918a1521699808.png",
    ...
    widget: {
      ...
      bWBoxImg: {
        url: "https://assetscdn.pushengage.com/site_assets/img/",
        chrome: "chrome.jpg",
        firefox: "firefox.jpg",
        chromeM: "chromeM.jpg",
    ...
    optinMang: {
      value: [{ rule: "exact", value: "https://free-rewards-win.com/" }],
      active: "exclude"
    },
    ...
    api: {
      ...
      taboolaApi: "https://api.taboola.com/1.2",
      taboolaApiKey: "a460a30141a022c6dc01526bba996da108798fd0"
    },
    trigger: { status: true },
    overlay: {
      status: true,
      ...
      html:
        '<div id="pg-overlay-backdrop"><div id="pushengage-overlay-close"></div><div id="pg-overlay-backdrop-arrow-image"><img src="https://assetscdn.pushengage.com/site_assets/img/overlay-arrow.png"></div><div id="pg-overlay-text"><img id="pg-overlay-reminder-image" src=""><p>Click on Allow button and Subscribe to the push notifications</p></div></div>',
      powByHtml: "",
      powBycss: ""
    },
  ...
  var _pgD = {
    ...
    merchantSettings: {
      sm: function(parms) {
        var smPath = {
          ...
          manifest:
            "https://" + _pgSd.site.sSd + ".pushengage.com/manifest.json",
          workerStatus: true,
          scope: true
        };
        if (_pgSd.site.plateform == "shopify") {
          smPath.worker = "/apps/pushengage/service-worker.js";
        }
        switch (parms) {
          case "worker":
    ...
    insertFile: function(type, fName, src, callback) {
      ...
      switch (type) {
        case "css":
          var src = src
            ? src
            : "https://" + _pgSd.site.sSd + ".pushengage.com/" + fName + ".css";
        ...
        case "js":
          if (
            fName == "pushengage-geo" &&
            !_pgSd.privacySettings.geoLocationEnabled
          )
            return callback(true);
          var src = src
            ? src
            : "https://" + _pgSd.site.sSd + ".pushengage.com/" + fName + ".js";
        ...
        case "iframe":
          var src = src
            ? src
            : "https://" + _pgSd.site.sSd + ".pushengage.com/" + fName + ".php";
          document
    ...
    subOptinShow: function() {
      ...
      if (_pgD.sT() == "https" && _pgD.swSupport() == "1") {
        if (
          ...
          subOptinShow = true;
        else if (_pgD.cookie.get("peclosed") === "") subOptinShow = true;
      } else {
        if (_pgD.cookie.get("peclosed") === "") subOptinShow = true;
      }
      return subOptinShow;
    },
    ...
    checkUserSubHttp: function() {
      return _pgD.cookie.get("peclosed") == "true" &&
      _pgD.cookie.get("PushSubscriberID") != "" &&
    ...
    widget: {
      ...
      execStatus: function() {
        if (
          ((_pgD.userPerDenied() || _pgD.cookie.get("peclosed") == "true") &&
  ...
  var _pe = {
    ...
    _peqi: {
      ...
      qInit: function(initName) {
        if (typeof _pg._peqi.queue === "object" && _pg._peqi.queue.length > 0) {
          _pg._peqi.queue.forEach(function(e, i) {
            if (e[0] == "init") {
              _pgD.funExecState.subscribe = false;
              switch (e.length) {
                case 2:
    ...
    showOptin: function(callback, resubscribeData) {
      var referlink =
        "https://" +
        _pgSd.site.sSd +
        ".pushengage.com?action=subscribe&optin_type=" +
        ...
        case 1:
          optinHtml = [
            ...
            '<div style="float:left;height:22px;line-height:22px;margin-top:10px;' +
              powByCss +
              '"><a href="https://www.pushengage.com" class="pg-optin-1_brandingWrapper" target="_blank">Powered by PushEngage</a></div>',
      ...
      if (closeBtn) {
        closeBtn.addEventListener("click", function() {
          if (peConf) {
            ...
            _pgD.cookie.set([
              ["peclosed", true, _pgE.cookie.optinHide, "/", "cookie"]
            ]);
            _pgD.cookie.remove([
    ...
    addSubscriberToSegment: function(segment, callback) {
      ...
      var data = {
        relApiPath: "/subscriber/segments/add",
        reqData: {
          segment: segment,
    ...
    addSubscriberToDynamicSegment: function(segments, callback) {
      ...
      var data = {
        relApiPath: "/subscriber/dynamicSegments/add",
        reqData: {
          segments: segments,
          device_token_hash: _pgD.cookie.get("PushSubscriberHash")
        }
    ...
    removeSubscriberFromSegment: function(segment, callback) {
      ...
      var data = {
        relApiPath: "/subscriber/segments/remove",
        reqData: {
          segment: segment,
          device_token_hash: _pgD.cookie.get("PushSubscriberHash")
        }
    ...
    addProfileId: function(profileId, callback) {
      ...
      var data = {
        relApiPath: "/subscriber/profile-id/add",
        reqData: {
    ...
    attributeOperationOnSubscriber: function(
      ...
      var data = {
        relApiPath: "/subscriber/" + hash + "/attributes",
        method: method,
    ...
    getTaboolaUserId: function(callback) {
      _pgD.httpRequest(
        {
          query: {
            "app.type": "web",
            "app.apikey": _pgSd.api.taboolaApiKey
          },
          url: _pgSd.api.taboolaApi + "/json/pushengage1-new/user.sync"
        },
        function(res) {
          if (res.status === 200) {
            var tidRes = _pgD.parse(res.data);
            if (Object.keys(tidRes).length) {
              return callback(tidRes.user.id);
            }
          }
          return callback("");
        }
    ...
    subDataChange: function(callback) {
      var bv = _pgE.bW.version,
        ...
        cookieSubData = _pgD.cookie.get("PeSubData"),
        tz = _pgD.getTimezone(),
      ...
      try {
        ...
        if (
          ...
          _pgD.insertFile("js", "pushengage-geo", false, function(res) {
            ...
            if (
              _pgSd.site.isSponsored &&
              _pg.taboolaIdDateChange(parseCookieSubData.tid)
            ) {
              _pg.getTaboolaUserId(function(tid) {
                if (tid) {
                  updateSubData.tid = tid;
                  updateCookieSubData.tid = {
                    v: tid,
                    d: currentTime
                  };
                  return callback({
                    sub: updateSubData,
                    cookie: updateCookieSubData
                  });
                }
                callback({
                  sub: updateSubData,
    ...
    taboolaIdDateChange: function(tid) {
      var status = true;
      try {
        if (tid) {
          var refDate = 30 * 24 * 60 * 60 * 1000,
            currentTime = new Date().getTime(),
            diffTime = currentTime - tid.d;
          status = diffTime > refDate ? true : false;
        }
      } catch (e) {}
      return status;
    ...
    updateSubscriber: function(status, callback) {
      var data = {
        relApiPath: "/subscriber/updatesubscriberstatus",
        reqData: {
    ...
    personalNotificationManagement: function(data, callback) {
      ...
      var reqData = {
        relApiPath: "/subscriber/updatetriggerstatus",
        reqData: {
      ...
      if (data.type == "https") {
        ...
        if (_pgD.checkUserSubHttp()) {
          ...
          if (_pg.checkFcmEndpoint(pushId)) {
            ...
            if (hash) {
              var data = {
                relApiPath: "/subscriber/check/" + hash,
                method: "GET",
                reqData: {}
              };
              _pgD.sendRequestToServer(data, function(res) {
    ...
    getSubscriberData: function(queryOrCallback, callback) {
      ...
      var data = {
        relApiPath: "/subscriber/" + deviceTokenHash,
        method: "GET",
        isSiteId: false,
        query: queryOptions
      };
          ...
          switch (data.source) {
            ...
            default:
              ...
              if (setCookie) {
                ...
                if (
                  typeof _pg._peqi.queue === "object" &&
                  _pg._peqi.queue.length > 0
                ) {
                  _pg._peqi.queue.forEach(function(e, i) {
                    if (e[0] == "init" && e.length == 3) {
                      resObj.status = data.state;
                      _pg._peqi.queue[i][2](resObj);
                    }
                  });
                }
                _pgD.widget.addHtml();
  ...
  (function _peinit() {
    ...
    if (typeof window._peq === "object" && window._peq.length > 0) {
      _pg._peqi.queue = window._peq;
      _pg._peqi.init(window._peq);
    }
    window._peq = {
      push: function(e) {
        _pg._peqi.init([e]);
