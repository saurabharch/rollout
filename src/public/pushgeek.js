try {
  var pg = {
    ...
    overlay: {
        ...
        if (!overlayId && _pgE.dT == "desktop") {
          if (_pgSd.overlay.status) {
            ...
            if (_pgE.bW.name == "Edge" && _pgE.bW.version < "79") {
              var oNT = _pgD.getId("pe-pc-notice-text");
            ...
            if (_pgD.getInputUrl("intermediate")) {
              var id = _pgD.getId("pe-overlay-powby");
              if (id)
                id.href =
      ...
      hideHtml: function() {
        _pgD.rvHtml(_pgD.getId("pe-overlay-powby"));
        _pgD.rvHtml(_pgD.getId("pe-overlay-backdrop"));
        _pgD.rvHtml(_pgD.getId("pe-overlay-backdrop-style"));
      }
    },
    apiValid: function() {
      var valid = true;
    ...
    permissionGranted: function(subscription, maincallback) {
      ...
      _pgD.insertFile("js", "pushgeek-geo", false, function(res) {
        var ip = false;
        var geoInfo;
        if (typeof peGeoInfo == "undefined") geoInfo = _pgD.modifyGeo();
        else {
          geoInfo = _pgD.modifyGeo(peGeoInfo);
          ip = geoInfo.geobytesipaddress;
        }
        var availScreen = screen.availWidth + "*" + screen.availHeight;
        var data = {
          relApiPath: "/subscriber/add",
          reqData: {
        ...
        _pgD.sendRequestToServer(data, function(res) {
          ...
          if (
            ...
            _pgD.resetStorage();
            _pgD.cookie.set([
              ["peclosed", true, _pgD.cookie.global, "/", "cookie"],
    ...
    sendDataToBrowser: function() {
      ...
      var data = {
        ...
        subCookieData: _pgD.cookie.get("PeSubData")
      };
      if (window.opener) {
    ...
    formatGeoInfo: function() {
      return {
        country:
          typeof peGeoInfo != "undefined" &&
          typeof peGeoInfo.geobytescountry != "undefined"
            ? peGeoInfo.geobytescountry
            : "",
        state:
          typeof peGeoInfo != "undefined" &&
          typeof peGeoInfo.geobytesregion != "undefined"
            ? peGeoInfo.geobytesregion
            : "",
        city:
          typeof peGeoInfo != "undefined" &&
          typeof peGeoInfo.geobytescity != "undefined"
            ? peGeoInfo.geobytescity
            : ""
      };
    },
    ...
    optinSubscriptionReminder: {
      ...
      reminder: function() {
        ...
        var imageUrl =
          "https://assetscdn.pushgeek.com/site_assets/img/reminder-chrome.png";
        if (_pgE.bW.name === "Firefox") {
          imageUrl =
            "https://assetscdn.pushgeek.com/site_assets/img/reminder-firefox.png";
        }
        try {
          var cssContent =
