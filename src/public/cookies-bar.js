...
var pushEngageCookieOperation = {
  ...
  insertFile: function(callback) {
    ...
    var src = src ? src : "https://demo.pushengage.com/pushengage-geo.js";
...
$("#cookie-warning-bottom-navigation__ok-btn").on("click", function() {
  pushEngageCookieOperation.cookie.set([["peCookieWarning", false, 180, "/"]]);
  hideBottomNavigation();
});
...
$(document).ready(function() {
  if (pushEngageCookieOperation.cookie.get("peCookieWarning") != "false") {
    pushEngageCookieOperation.insertFile(function(res) {
      if (
        res &&
        $.inArray(peGeoInfo.geobytescountry, allowCookieWarningCountry) != -1
      ) {
        cookieWarning.show();
      }
    });
